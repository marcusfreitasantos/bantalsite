...
if ( $expiring_today ) {
	...
	echo wp_kses_post( sprintf( __( 'A vaga expira hoje <a href="%1$s">%2$s</a>.', 'wp-job-manager' ), esc_url( home_url() ), esc_html( get_bloginfo( 'name' ) ) ) );
} else {
	// translators: %1$s placeholder is URL to the blog. %2$s placeholder is the name of the site.
	echo wp_kses_post( sprintf( __( 'A vaga expira em breve <a href="%1$s">%2$s</a>.', 'wp-job-manager' ), esc_url( home_url() ), esc_html( get_bloginfo( 'name' ) ) ) );
}

echo ' ';

// translators: Placeholder is URL to site's WP admin.
echo wp_kses_post( sprintf( __( 'Acesse <a href="%s">o painel</a> para gerenciar as vagas.', 'wp-job-manager' ), esc_url( $edit_post_link ) ) );
echo '</p>';

...
if ( $expiring_today ) {
	echo wp_kses_post(
		sprintf(
			__( 'A vaga expira em <a href="%s">%s</a>.', 'wp-job-manager' ),
			home_url(),
			get_bloginfo( 'name' )
		)
	);
	...
	echo wp_kses_post(
		sprintf(
			__( 'A vaga expira em breve <a href="%s">%s</a>.', 'wp-job-manager' ),
			home_url(),
			get_bloginfo( 'name' )
		)
	);
...
echo wp_kses_post(
	sprintf(
		' ' . __( 'Acesse <a href="%s">o painel </a> para gerenciar suas vagas.', 'wp-job-manager' ),

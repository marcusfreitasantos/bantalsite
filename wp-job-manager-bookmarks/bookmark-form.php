...
<form method="post" action="<?php echo defined( 'DOING_AJAX' ) ? '' : esc_url( remove_query_arg( array( 'page', 'paged' ), add_query_arg( $wp->query_string, '', home_url( $wp->request ) ) ) ); ?>" class="job-manager-form wp-job-manager-bookmarks-form <?php echo $is_bookmarked ? 'has-bookmark' : ''; ?>">
	<div class="remove-bookmark-wrapper">
		<a class="remove-bookmark" href="<?php echo wp_nonce_url( add_query_arg( 'remove_bookmark', absint( $post->ID ), get_permalink() ), 'remove_bookmark' ); ?>"><?php _e( 'Remove Bookmark', 'wp-job-manager-bookmarks' ); ?></a> <a class="bookmark-notice bookmarked" href="#"><?php printf( __( 'This %s is bookmarked!', 'wp-job-manager-bookmarks' ), $post_type->labels->singular_name ); ?></a>
	</div>
	
	<div class="add-bookmark-wrapper">
		<a class="bookmark-notice" href="#"><?php printf( __( 'Bookmark This %s', 'wp-job-manager-bookmarks' ), ucwords( $post_type->labels->singular_name ) ); ?></a>
	</div>

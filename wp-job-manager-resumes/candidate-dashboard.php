...
<div id="resume-manager-candidate-dashboard">
	<p><?php echo _n( 'Seu currículo pode ser visualizado, editado ou removido abaixo.', 'Seus currículos podem ser visualizados, editados ou removidos abaixo.', resume_manager_count_user_resumes(), 'wp-job-manager-resumes' ); ?></p>
	<table class="resume-manager-resumes">
		...
		<tbody>
			<?php if ( ! $resumes ) : ?>
				<tr>
					<td colspan="<?php echo sizeof( $candidate_dashboard_columns ); ?>"><?php _e( 'Você não tem currículos cadastrados.', 'wp-job-manager-resumes' ); ?></td>
				</tr>
			<?php else : ?>
									...
									<ul class="candidate-dashboard-actions">
										<?php
											...
											switch ( $resume->post_status ) {
												case 'publish' :
													$actions['edit'] = array( 'label' => __( 'Editar', 'wp-job-manager-resumes' ), 'nonce' => false );
													$actions['hide'] = array( 'label' => __( 'Esconder', 'wp-job-manager-resumes' ), 'nonce' => true );
												break;
												case 'hidden' :
													$actions['edit'] = array( 'label' => __( 'Editar', 'wp-job-manager-resumes' ), 'nonce' => false );
													$actions['publish'] = array( 'label' => __( 'Publicar', 'wp-job-manager-resumes' ), 'nonce' => true );
												break;
												case 'expired' :
											...
											$actions['delete'] = array( 'label' => __( 'Deletar', 'wp-job-manager-resumes' ), 'nonce' => true );
			...
			<tfoot>
				<tr>
					<td colspan="<?php echo sizeof( $candidate_dashboard_columns ); ?>">
						<a href="<?php echo esc_url( get_permalink( $submit_resume_form_page_id ) ); ?>"><?php _e( 'Cadastrar Novo Currículo', 'wp-job-manager-resumes' ); ?></a>
					</td>
				</tr>
			</tfoot>
		<?php endif; ?>

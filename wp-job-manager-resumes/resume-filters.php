...
<form class="resume_filters">
	...
	<div class="search_resumes">
		...
		<?php if ( $categories ) : ?>
			...
			<?php endforeach; ?>
		<?php elseif ( $show_categories && get_option( 'resume_manager_enable_categories' ) && ! is_tax( 'rea_de_atuao' ) && get_terms( 'rea_de_atuao' ) ) : ?>
			<div class="search_categories resume-filter">
				<label for="search_categories"><?php _e( 'Category', 'wp-job-manager-resumes' ); ?></label>
				<?php if ( $show_category_multiselect ) : ?>
					<?php job_manager_dropdown_categories( array( 'taxonomy' => 'rea_de_atuao', 'hierarchical' => 1, 'name' => 'search_categories', 'orderby' => 'name', 'selected' => $selected_category, 'hide_empty' => false ) ); ?>
				<?php else : ?>
					<?php wp_dropdown_categories( array( 'taxonomy' => 'rea_de_atuao', 'hierarchical' => 1, 'show_option_all' => __( 'Any category', 'wp-job-manager-resumes' ), 'name' => 'search_categories', 'orderby' => 'name', 'selected' => $selected_category ) ); ?>
				<?php endif; ?>
			</div>
		<?php endif; ?>

...
echo wp_kses_post( sprintf( __( 'Um novo currículo foi cadastrado em <a href="%1$s">%2$s</a>.', 'wp-job-manager-resumes' ), home_url(), get_bloginfo( 'name' ) ) );

switch ( $resume->post_status ) {
	case 'publish':
		echo ' ' . esc_html__( 'Está públicado e disponível para o público.', 'wp-job-manager-resumes' );
		break;
	case 'pending':
		// translators: Placeholder is URL for WP admin.
		echo ' ' . wp_kses_post( sprintf( __( 'Está aguardando aprovação em <a href="%s">WordPress admin</a>.', 'wp-job-manager-resumes' ), esc_url( admin_url( 'edit.php?post_type=resume' ) ) ) );
		break;
}

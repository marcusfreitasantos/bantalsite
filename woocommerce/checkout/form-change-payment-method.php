...
<form id="order_review" method="post">
	...
	<table class="shop_table">
		<thead>
			<tr>
				<th class="product-name"><?php echo esc_html_x( 'Produto', 'table headings in notification email', 'woocommerce-subscriptions' ); ?></th>
				<th class="product-quantity"><?php echo esc_html_x( 'Quantidade', 'table headings in notification email', 'woocommerce-subscriptions' ); ?></th>
	...
	<div id="payment">
		...
		if ( $subscription->has_payment_gateway() ) {
			$pay_order_button_text = _x( 'Alterar método de pagamento', 'text on button on checkout page', 'woocommerce-subscriptions' );
		} else {
			$pay_order_button_text = _x( 'Adicionar método de pagamento', 'text on button on checkout page', 'woocommerce-subscriptions' );
		}
		...
		<?php else : ?>
			<div class="woocommerce-error">
				<p> <?php echo esc_html( apply_filters( 'woocommerce_no_available_payment_methods_message', __( 'Desculpe, este método de pagamento não está disponível.', 'woocommerce-subscriptions' ) ) ); ?></p>
			</div>
		<?php endif; ?>
		...
		<?php if ( $available_gateways ) : ?>
			...
			<?php
				...
				$label = sprintf( esc_html__( 'Atualizar método de pagamento usado para %1$sall%2$s de minhas assinaturas', 'woocommerce-subscriptions' ), '<strong>', '</strong>' );

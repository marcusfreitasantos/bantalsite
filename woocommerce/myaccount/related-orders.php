	...
	<tbody>
		<?php foreach ( $subscription_orders as $subscription_order ) :
			...
			?><tr class="order woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?>">
				<td class="order-number woocommerce-orders-table__cell woocommerce-orders-table__cell-order-number" data-title="<?php esc_attr_e( 'Número do Pedido', 'woocommerce-subscriptions' ); ?>">
				...
				<td class="order-date woocommerce-orders-table__cell woocommerce-orders-table__cell-order-date" data-title="<?php esc_attr_e( 'Data', 'woocommerce-subscriptions' ); ?>">
				...
				<td class="order-total woocommerce-orders-table__cell woocommerce-orders-table__cell-order-total" data-title="<?php echo esc_attr_x( 'Total', 'Used in data attribute. Escaped', 'woocommerce-subscriptions' ); ?>">
					...
					echo wp_kses_post( sprintf( _n( '%1$s para %2$d item', '%1$s para %2$d items', $item_count, 'woocommerce-subscriptions' ), $order->get_formatted_order_total(), $item_count ) );
					?>
				</td>
				<td class="order-actions woocommerce-orders-table__cell woocommerce-orders-table__cell-order-actions">
					...
					if ( $order->needs_payment() && wcs_get_objects_property( $order, 'id' ) == $subscription->get_last_order( 'ids', 'any' ) ) {
						$actions['pay'] = array(
							...
							'name' => esc_html_x( 'Pagar', 'pagar por uma assinatura', 'woocommerce-subscriptions' ),
						);
					}
					...
					$actions['view'] = array(
						...
						'name' => esc_html_x( 'Detalhes', 'view a subscription', 'woocommerce-subscriptions' ),
					);

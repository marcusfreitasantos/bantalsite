...
<table class="shop_table subscription_details">
	<tbody>
		...
		$dates_to_display = apply_filters( 'wcs_subscription_details_table_dates_to_display', array(
			'start_date'              => _x( 'Data de início', 'customer subscription table header', 'woocommerce-subscriptions' ),
			'last_order_date_created' => _x( 'Data do último pagamento', 'customer subscription table header', 'woocommerce-subscriptions' ),
			'next_payment'            => _x( 'Próximo pagamento', 'customer subscription table header', 'woocommerce-subscriptions' ),
			'end'                     => _x( 'Vencimento', 'customer subscription table header', 'woocommerce-subscriptions' ),
			'trial_end'               => _x( 'Período de teste', 'customer subscription table header', 'woocommerce-subscriptions' ),
		), $subscription );
		...
		<?php if ( WCS_My_Account_Auto_Renew_Toggle::can_subscription_auto_renewal_be_changed( $subscription ) ) : ?>
			<tr>
				<td><?php esc_html_e( 'Renovação automática', 'woocommerce-subscriptions' ); ?></td>
				<td>
					<div class="wcs-auto-renew-toggle">
						...
						if ( $subscription->is_manual() ) {
							$toggle_label     = __( 'Ativar renovação automática', 'woocommerce-subscriptions' );
							...
							$toggle_label     = __( 'Desativar renovação automática', 'woocommerce-subscriptions' );
						...
						<?php if ( WC_Subscriptions::is_duplicate_site() ) : ?>
								<small class="subscription-auto-renew-toggle-disabled-note"><?php echo esc_html__( 'O uso da renovação automática é desativado no modo de preparação.', 'woocommerce-subscriptions' ); ?></small>
						<?php endif; ?>
					</div>
				</td>
			</tr>
		...
		<?php if ( $subscription->get_time( 'next_payment' ) > 0 ) : ?>
			<tr>
				<td><?php esc_html_e( 'Pagamento', 'woocommerce-subscriptions' ); ?></td>
				<td>
		...
		<?php if ( ! empty( $actions ) ) : ?>
			<tr>
				<td><?php esc_html_e( 'Ações', 'woocommerce-subscriptions' ); ?></td>
				<td>
